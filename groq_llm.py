...
Uses Groq API for text generation with Llama models.
...
class GroqLLM:
    ...
    def __init__(
        ...
        if not self.api_key:
            raise ValueError("Groq API key not provided. Set $env:GROQ_API_KEY or pass api_key parameter.")
    ...
    def _call_api(self, messages: List[Dict[str, str]], stream: bool = False) -> str:
        """
        ...
        for attempt in range(1, self.max_retries + 1):
            try:
                ...
                if attempt < self.max_retries:
                    import time
                    wait = 2 * attempt
                    print(f"Groq API error ({e}); retrying in {wait}s...")
                    time.sleep(wait)
                    continue
...
Roman Urdu output (ONLY Latin letters):"""
    ...
    def generate_response_with_context(
        ...
        Args:
            ...
            is_roman_mode: Whether to force Roman Urdu output
...
if __name__ == "__main__":
    ...
    if not api_key:
        ...
        print("\nTesting Groq LLM...")
        response = llm.generate_response("Explain RAG architecture briefly in 2-3 sentences.")
        print(f"\nResponse: {response}")
